...
export function decodeBuffer(buffer, key) {
	...
	for (let i=0; i<buffer.length; i++) {
		tmpBuffer[i] = (256+(buffer[i]-key)%256)%256;
	}
...
export function getMaxFrameSize() {
	return 2889;
}
...
export function resolveFrameHeader(buffer) {
	if (buffer[0]===0xff && buffer[1]&0xe0) {
		...
		if (version==='x' || layer==='x') {
			return null;
		}

		const key = 'V' + (version=='2.5' ? '2' : version) + 'L' + layer;
		const bitrate = bitrates[key] && bitrates[key][(b2 & 0xf0) >> 4] || 0;
		...
		return { key, bitrate, sample_rate, samples, duration, size, offset };
	}
