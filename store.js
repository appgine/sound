...
export function preload(url, labels, label, userAction) {
		...
		return thisTrack = thisTrack || {
			...
			control: {
				...
				pause(seconds=0) {
					...
					timer.clearBackgroundTimeout(fadeInTimeout);
	...
	timer.setBackgroundTimeout(notify, 0);
	...
	const track = {
		...
		play(fadeOut, fadeIn) {
			...
			if (fadeOut>0) {
				fadeInTimeout = timer.setBackgroundTimeout(function() {
		...
		fade(seconds) {
			...
			if (currentTrack) {
				...
				timer.setBackgroundTimeout(function() {
